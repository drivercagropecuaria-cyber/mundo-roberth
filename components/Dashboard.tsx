...
interface DashboardStats {
  ...
  total_research: number; research_completed: number; total_sources: number; total_academic_sources: number; total_directives: number; total_dossiers: number; total_presentations: number; avg_quality: number;
  total_conflicts: number; total_notifications: number; avg_sources_per_research: number; memory_entries: number; traces: number; success_rate: number;
}
interface Job { id: string; status: string; intent: string|null; action: string|null; confidence: string|null; user_message: string|null; user_name: string|null; created_at: string; quality_score?: number; agent_version?: string; orchestration_log?: any[]; event_count?: number; }
...
interface RSource { title: string; url: string; source_platform: string; relevance_score: number; upvotes: number; comments_count: number; author: string; content_snippet: string; source_layer?: string; source_strength?: string; evidence_type?: string; confirmation_degree?: string; independence?: boolean; potential_bias?: string; observations?: string; academic_type?: string; authors?: string; institution?: string; publication_year?: number; quality_score?: number; authority_score?: number; }
...
interface RReport { id: string; topic: string; summary: string|null; executive_summary: string|null; main_answer: string|null; full_report: string|null; conflicts: string|null; timeline: string|null; next_searches: string|null; sources_count: number; status: string; mode: string|null; confidence: string|null; search_contract: any; self_evaluation: any; created_at: string; sources: RSource[]|null; conflict_details: RConflict[]|null; swot_analysis?: any; search_strategy?: any; academic_sources_count?: number; source_tiers?: any; thought_tree?: any; search_rounds?: number; coverage_score?: any; expansion_log?: any; metadata?: any; }
...
function Dossier({ r, onBack }: { r: RReport; onBack: () => void }) {
  ...
  const acad = src.filter(s => s.academic_type && ["paper","book","thesis","report","data"].includes(s.academic_type)).length;
  ...
  return <div style={{ fontFamily: "Inter, system-ui, sans-serif", color: V.text, lineHeight: 1.6, animation: "slideIn 0.4s ease" }}>
    ...
    <div style={{ position: "sticky", top: 0, zIndex: 50, backdropFilter: "blur(18px)", background: "rgba(8,16,30,0.72)", borderBottom: "1px solid " + V.line, padding: "14px 0", marginBottom: 8 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div><div style={{ fontWeight: 700 }}>Agente de Pesquisa</div><small style={{ color: V.muted }}>Motor v14</small></div>
        </div>
      </div>
    </div>
    ...
    <div style={{ border: "1px solid " + V.line, background: "linear-gradient(180deg, rgba(255,255,255,0.08), rgba(255,255,255,0.04))", borderRadius: 28, padding: 36, boxShadow: V.shadow, position: "relative", overflow: "hidden", marginBottom: 22 }}>
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
          ...
          {r.source_tiers && <span style={{ padding: "10px 14px", background: "rgba(124,156,255,0.1)", border: "1px solid rgba(124,156,255,0.25)", borderRadius: 999, color: "#dce5ff", fontSize: 13 }}>{"\ud83c\udf93"} T1: <strong>{r.source_tiers.tier1 || 0}</strong> {"\ud83c\udfdb"} T2: <strong>{r.source_tiers.tier2 || 0}</strong> {"\ud83d\udcf0"} T3: <strong>{r.source_tiers.tier3_4 || 0}</strong></span>}
          {r.thought_tree && <span style={{ padding: "10px 14px", background: "rgba(78,227,193,0.1)", border: "1px solid rgba(78,227,193,0.25)", borderRadius: 999, color: "#a7f3d0", fontSize: 13 }}>{"\ud83c\udf33"} {(r.thought_tree.primary_branches || []).length} ramos</span>}
          {r.search_rounds && <span style={{ padding: "10px 14px", background: "rgba(255,200,107,0.1)", border: "1px solid rgba(255,200,107,0.25)", borderRadius: 999, color: "#fde68a", fontSize: 13 }}>{"\ud83d\udd04"} {r.search_rounds} rounds</span>}
          {r.metadata?.method && <span style={{ padding: "10px 14px", background: "rgba(255,255,255,0.05)", border: "1px solid " + V.line, borderRadius: 999, color: V.muted, fontSize: 12 }}>{r.metadata.method}</span>}
        </div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 18, marginBottom: 22 }}>
      <Card><div style={{ color: V.muted, fontSize: 13, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 8 }}>Fontes</div><div style={{ fontSize: 40, fontWeight: 800 }}>{String(src.length).padStart(2, "0")}</div><div style={{ color: V.muted, fontSize: 14 }}>{"\ud83c\udf93"} {acad} acad. {"\ud83d\udcf0"} {sec} sec. {"\ud83d\udcac"} {tert} compl.</div></Card>
      ...
      <Card><div style={{ color: V.muted, fontSize: 13, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 8 }}>Rigor Acad.</div><div style={{ fontSize: 40, fontWeight: 800 }}>{ev.academic_rigor || ev.coverage || "?"}%</div><div style={{ color: V.muted, fontSize: 14 }}>Autoavalia\u00e7\u00e3o</div></Card>
    </div>

    {/* ARVORE DE PENSAMENTO */}
    {r.thought_tree && r.thought_tree.primary_branches && r.thought_tree.primary_branches.length > 0 && (
      <Card style={{ marginBottom: 22 }}>
        <h3 style={{ fontSize: 18, fontWeight: 700, margin: "0 0 14px" }}>{"\ud83c\udf33"} \u00c1rvore de Pensamento</h3>
        <div style={{ color: V.muted, fontSize: 13, marginBottom: 12 }}>Tronco: <strong style={{ color: V.text }}>{r.thought_tree.trunk || r.topic}</strong></div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: 12 }}>
          {(r.thought_tree.primary_branches || []).map((b: any, i: number) => (
            <div key={i} style={{ padding: "14px 16px", borderRadius: 14, border: "1px solid " + V.line, background: b.priority === "essential" ? "rgba(124,156,255,0.06)" : "rgba(255,255,255,0.03)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                <strong style={{ color: V.text, fontSize: 13 }}>{b.name}</strong>
                <span style={{ fontSize: 11, color: b.priority === "essential" ? "#7c9cff" : V.muted, background: b.priority === "essential" ? "rgba(124,156,255,0.15)" : "rgba(255,255,255,0.06)", padding: "2px 8px", borderRadius: 999 }}>{b.priority}</span>
              </div>
              <div style={{ fontSize: 11, color: V.muted, marginBottom: 4 }}>{b.ideal_source_type || ""}</div>
              {b.secondary_branches && b.secondary_branches.length > 0 && (
                <div style={{ marginTop: 6 }}>
                  {b.secondary_branches.map((s: any, j: number) => (
                    <div key={j} style={{ fontSize: 11, color: V.muted, padding: "2px 0", borderLeft: "2px solid rgba(124,156,255,0.3)", paddingLeft: 8, marginBottom: 2 }}>{s.name}</div>
                  ))}
                </div>
              )}
            </div>
    ...
    {src.length > 0 && <Card style={{ padding: 0, overflow: "hidden", marginBottom: 18 }}>
      <div style={{ overflowX: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 900 }}>
          <thead><tr>{["Fonte", "Tipo", "Camada", "For\u00e7a", "Evid\u00eancia", "Autor/Institui\u00e7\u00e3o", "Vi\u00e9s"].map(h => (
          ...
          <tbody>{src.map((s, i) => {
            const acType: Record<string,string> = { paper: "\ud83c\udf93 Paper", book: "\ud83d\udcda Livro", thesis: "\ud83c\udf93 Tese", report: "\ud83c\udfdb Relat\u00f3rio", data: "\ud83d\udcca Dados", journalism: "\ud83d\udcf0 Jornal.", technical: "\ud83d\udd27 T\u00e9cnico", opinion: "\ud83d\udcac Opini\u00e3o" };
            return <tr key={i} style={{ borderBottom: "1px solid " + V.line }}>
              <td style={{ padding: "12px 16px", fontSize: 13 }}>{s.url ? <a href={s.url} target="_blank" rel="noopener" style={{ color: V.accent, textDecoration: "underline" }}>{(s.title || "").substring(0, 50)}</a> : (s.title || "").substring(0, 50)}</td>
              <td style={{ padding: "12px 16px", fontSize: 12, color: V.muted }}>{acType[s.academic_type || ""] || (layerE[s.source_layer || ""] || "") + " " + (s.source_layer || "\u2014")}</td>
              ...
              <td style={{ padding: "12px 16px", fontSize: 12, color: V.muted }}>{s.authors || s.institution || "\u2014"}</td>
    ...
    <div style={{ display: "grid", gridTemplateColumns: conf_.length > 0 ? "1.4fr 0.9fr" : "1fr", gap: 18, marginBottom: 22 }}>
      <Card><h3 style={{ fontSize: 18, fontWeight: 700, margin: "0 0 14px" }}>Fontes Comentadas</h3>
        {src.slice(0, 8).map((s, i) => {
          ...
          const acIc: Record<string, string> = { paper: "\ud83c\udf93", book: "\ud83d\udcda", thesis: "\ud83c\udf93", report: "\ud83c\udfdb", data: "\ud83d\udcca", journalism: "\ud83d\udcf0", technical: "\ud83d\udd27" };
          return <div key={i} style={{ padding: "14px 16px", borderRadius: 18, border: "1px solid " + V.line, background: "rgba(255,255,255,0.04)", marginBottom: 10 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6, flexWrap: "wrap" }}>
              <div style={{ fontWeight: 700 }}>{s.url ? <a href={s.url} target="_blank" rel="noopener" style={{ color: V.accent }}>{(s.title || "").substring(0, 60)} {"\u2197"}</a> : (s.title || "").substring(0, 60)}</div>
              <div style={{ color: V.muted, fontSize: 12 }}>{acIc[s.academic_type || ""] || ""} {s.academic_type || lL[s.source_layer || ""] || ""} {"\u2022"} {s.source_strength || ""}</div>
            </div>
            {s.authors && <div style={{ color: "#ffc86b", fontSize: 12, marginBottom: 4 }}>{"\ud83d\udc64"} {s.authors}{s.institution ? " \u2022 " + s.institution : ""}</div>}
  ...
  return <div style={{ minHeight: "100vh", color: "#fff", background: "#09090b", position: "relative", overflow: "hidden", fontFamily: "Inter, system-ui, sans-serif" }}>
    ...
    <header className="no-print" style={{ position: "relative", zIndex: 10, padding: "24px 32px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
        ...
        <div><div style={{ fontSize: 18, fontWeight: 800, letterSpacing: -0.5 }}>MUNDO ROBERTH</div><div style={{ fontSize: 11, color: V.muted, fontWeight: 600, letterSpacing: 2, textTransform: "uppercase" }}>Dashboard v0.7</div></div>
      </div>
      <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
    ...
    <main style={{ position: "relative", zIndex: 10, padding: "24px 32px", animation: "slideIn 0.3s ease" }}>
      ...
      {tab === "overview" && stats && <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 14 }}>
          ...
          <StatCard label="Acad\u00eamicas" value={stats.total_academic_sources || 0} icon="\ud83c\udf93" accent="rgba(255,200,107,0.08)" />
          <StatCard label="Dossi\u00eas" value={stats.total_dossiers || 0} icon="\ud83d\udcda" accent="rgba(124,156,255,0.08)" />
          <StatCard label="Apresenta\u00e7\u00f5es" value={stats.total_presentations || 0} icon="\ud83d\udcca" accent="rgba(255,125,125,0.08)" />
          <StatCard label="Qualidade" value={stats.avg_quality || 0} icon="\u2b50" accent="rgba(255,200,107,0.08)" />
          <StatCard label="Sucesso" value={stats.success_rate || 0} icon="\ud83d\udcc8" accent="rgba(111,226,163,0.08)" />
          <StatCard label="M\u00e9dia Fontes" value={stats.avg_sources_per_research || 0} icon="\ud83d\udcca" accent="rgba(124,156,255,0.08)" />
          <StatCard label="Mem\u00f3ria" value={stats.memory_entries || 0} icon="\ud83e\udde0" accent="rgba(78,227,193,0.08)" />
        </div>
      ...
      {tab === "jobs" && <GPanel title={"Jobs (" + jobs.length + ")"} icon="\ud83d\udd04" full><div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}><thead><tr style={{ borderBottom: "1px solid rgba(255,255,255,0.08)" }}>{["Status", "Mensagem", "A\u00e7\u00e3o", "Qualidade", "Agente", "Data"].map(h => <th key={h} style={{ padding: "10px 8px", textAlign: "left", color: "rgba(255,255,255,0.35)", fontSize: 11, fontWeight: 700, textTransform: "uppercase" }}>{h}</th>)}</tr></thead><tbody>{jobs.map((j, i) => <tr key={j.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.03)", animation: "slideIn 0.3s ease " + (i * 0.03) + "s both" }}><td style={{ padding: "10px 8px" }}><SBadge s={j.status} /></td><td style={{ padding: "10px 8px", color: "rgba(255,255,255,0.6)", maxWidth: 250, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{j.user_message ? String(j.user_message).replace(/"/g, "") : "\u2014"}</td><td style={{ padding: "10px 8px", color: "rgba(124,156,255,0.8)", fontWeight: 600 }}>{j.action || j.intent || "\u2014"}</td><td style={{ padding: "10px 8px" }}>{j.quality_score ? <span style={{ fontSize: 12, fontWeight: 700, color: j.quality_score >= 0.8 ? V.success : j.quality_score >= 0.6 ? V.warning : V.danger }}>{(j.quality_score * 100).toFixed(0)}%</span> : "\u2014"}</td><td style={{ padding: "10px 8px", color: "rgba(255,255,255,0.3)", fontSize: 11 }}>{j.agent_version || "\u2014"}</td><td style={{ padding: "10px 8px", color: "rgba(255,255,255,0.35)", fontSize: 11 }}>{fmtDate(j.created_at)}</td></tr>)}</tbody></table></div></GPanel>}
    ...
    <footer className="no-print" style={{ position: "relative", zIndex: 10, padding: "16px 32px", textAlign: "center", borderTop: "1px solid rgba(255,255,255,0.04)" }}>
      <span style={{ fontSize: 11, color: "rgba(255,255,255,0.15)" }}>Mundo Roberth v0.7 \u00b7 Motor v14 \u00b7 7 Agentes \u00b7 Auto-refresh 10s</span>
    </footer>
  </div>;
}
